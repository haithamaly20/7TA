...
APP.printing = (function(){
  ...
  function normalizePlanData(monthKey){
    ...
    supervisors.forEach(sup => {
      // ⭐ نجرب المفتاح كـ string وكـ number لضمان التطابق بغض النظر عن نوع البيانات
      const dayMap = plan[String(sup.id)] || plan[sup.id] || {};
      days.forEach(day => {
        const instituteId = dayMap[String(day.day)] ?? dayMap[day.day];
  ...
  function generalHtml(data){
    ...
    const workingDays = h().getMonthDays(data.monthKey, APP.storage.getSettings().weekendDows)
    ...
    // rowsMap[supId][day] = instituteName
    const rowsMap = {};
    data.rows.forEach(r => {
    ...
    // الأيام التي فيها أي زيارة مخططة
            ...
            <tbody>
              ${allDays.map(d => {
                const cells = chunk.map(sup => {
                  const name = (rowsMap[String(sup.id)] || {})[String(d.day)] || '';
